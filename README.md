...
打印实现采用固定 `66mm × 28mm` 标签、每页 3 列 × 9 行，共 27 个。打印网格本体为 `202mm × 276mm`，通过 `translateY(15mm)` 在打印渲染阶段强制下移，因此第一排标签距 A4 顶部 15mm、底部仍约留 6mm；左右各留 4mm。标签之间横向间距为 2mm、纵向间距为 3mm，每个标签使用统一的 `0.5pt` 黑色边框。打印根容器总高度限制为 296mm，避免浏览器毫米换算舍入产生空白第二页。
