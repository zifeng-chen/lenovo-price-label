...
打印实现采用固定 `66mm × 28mm` 标签、每页 3 列 × 9 行，共 27 个。打印网格本体为 `202mm × 276mm`，使用绝对位置固定在 A4 纸内：左右各留 4mm，顶部为 `10mm + 5px`（约 11.32mm），底部约留 9.68mm。标签之间横向间距为 2mm、纵向间距为 3mm，每个标签使用统一的 `0.5pt` 黑色边框。打印根容器总高度限制为 296mm，避免浏览器毫米换算舍入产生空白第二页。
