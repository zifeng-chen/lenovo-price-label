...
打印实现采用固定 `68mm × 28mm` 标签、每页 3 列 × 9 行，共 27 个。三列标签总宽度为 204mm，A4 页面左右各留 3mm；纵向间距为 3mm，顶部留 10mm、底部约留 11mm，因此标签不会贴到纸张的任何边缘。打印内容总高度限制为 296mm，额外的 1mm 可避免浏览器毫米换算舍入产生空白第二页。标签之间使用统一的 `0.5pt` 黑色裁切边框分隔。
