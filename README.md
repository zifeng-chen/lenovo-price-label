...
- 按品类、商品名称或价格即时筛选
- 表头全选仅作用于当前筛选结果
- 打印勾选数量不受限制，每 24 个商品自动增加一张 A4
- A4 横向分页打印，每页 4 列 × 6 行
...
1. 勾选需要打印的商品，数量不受限制
2. 点击“打印价格标签（数量）”
3. 系统按商品列表顺序每 24 个商品生成一张 A4
4. 在浏览器打印对话框选择 A4、横向、100%
5. 核对预览页数应为 `向上取整(商品数 ÷ 24)`
6. 正式打印前先打印测试页并测量物理尺寸
...
| 选择总数 | 不限制 |
| 每页数量 | 最多 24 个，超出后自动分页 |
| 布局 | 每页 4 列 × 6 行 |
| 标签尺寸 | 70mm × 28mm |
| 打印网格 | `calc(280mm + 3px)` × `calc(168mm + 5px)`，约 280.79mm × 169.32mm |
| A4 左右留白 | 自动居中，各约 8.1mm |
| A4 上下留白 | 在 209mm 打印页内自动居中，各约 19.8mm |
| 标签横向间距 | 1px |
| 标签纵向间距 | 1px |
...
```text
横向：4 × 70mm + 3 × 1px = calc(280mm + 3px) ≈ 280.79mm
纵向：6 × 28mm + 5 × 1px = calc(168mm + 5px) ≈ 169.32mm
```
...
- [ ] 打印队列可以选择任意数量商品
- [ ] 每 24 个标签自动增加一张 A4，页数为 `向上取整(数量 ÷ 24)`
- [ ] 最后一页后没有额外空白页
- [ ] 实体标签测量为 70mm × 28mm
- [ ] 标签横向和纵向间距均为 1px
